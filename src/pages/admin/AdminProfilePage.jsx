    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <section className="bg-white border border-[#E0D9CE] rounded-sm p-6 sm:p-8 space-y-6">
        <div className="flex items-center gap-3 border-b border-[#EDE7DD] pb-4">
          <FontAwesomeIcon icon={faUser} className="w-5 h-5 text-[#2D5A3D]" />
          ...
          <div className="flex-1 w-full space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                <FontAwesomeIcon icon={faCamera} className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-[#6E7570]" />
      ...
      <section className="bg-white border border-[#E0D9CE] rounded-sm p-6 sm:p-8 space-y-5">
        <div className="flex items-center gap-3 border-b border-[#EDE7DD] pb-4">
          <FontAwesomeIcon icon={faKey} className="w-5 h-5 text-[#2D5A3D]" />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="sm:col-span-2">
            ...
            <div className="relative"><input type={visiblePasswords.current ? "text" : "password"} value={passwordData.currentPassword} onChange={(event) => setPasswordData((current) => ({ ...current, currentPassword: event.target.value }))} className="w-full px-3 pr-10 py-2.5 text-sm bg-[#FAF8F5] border border-[#C7BEB1] rounded-sm" /><button type="button" onClick={() => setVisiblePasswords((current) => ({ ...current, current: !current.current }))} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#6E7570]" aria-label={visiblePasswords.current ? "Hide current password" : "Show current password"}>{visiblePasswords.current ? <FontAwesomeIcon icon={faEyeSlash} className="w-4 h-4" /> : <FontAwesomeIcon icon={faEye} className="w-4 h-4" />}</button></div>
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[#566158] mb-1.5">New Password</label>
            <div className="relative"><input type={visiblePasswords.next ? "text" : "password"} value={passwordData.newPassword} onChange={(event) => setPasswordData((current) => ({ ...current, newPassword: event.target.value }))} minLength={8} className="w-full px-3 pr-10 py-2.5 text-sm bg-[#FAF8F5] border border-[#C7BEB1] rounded-sm" /><button type="button" onClick={() => setVisiblePasswords((current) => ({ ...current, next: !current.next }))} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#6E7570]" aria-label={visiblePasswords.next ? "Hide new password" : "Show new password"}>{visiblePasswords.next ? <FontAwesomeIcon icon={faEyeSlash} className="w-4 h-4" /> : <FontAwesomeIcon icon={faEye} className="w-4 h-4" />}</button></div>
          </div>
          <div>
            <label className="block text-xs font-semibold uppercase tracking-wider text-[#566158] mb-1.5">Confirm New Password</label>
            <div className="relative"><input type={visiblePasswords.confirm ? "text" : "password"} value={passwordData.confirmPassword} onChange={(event) => setPasswordData((current) => ({ ...current, confirmPassword: event.target.value }))} minLength={8} className="w-full px-3 pr-10 py-2.5 text-sm bg-[#FAF8F5] border border-[#C7BEB1] rounded-sm" /><button type="button" onClick={() => setVisiblePasswords((current) => ({ ...current, confirm: !current.confirm }))} className="absolute right-2 top-1/2 -translate-y-1/2 text-[#6E7570]" aria-label={visiblePasswords.confirm ? "Hide password confirmation" : "Show password confirmation"}>{visiblePasswords.confirm ? <FontAwesomeIcon icon={faEyeSlash} className="w-4 h-4" /> : <FontAwesomeIcon icon={faEye} className="w-4 h-4" />}</button></div>
          </div>
        </div>
      </section>
